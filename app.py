...
def api_class_counts():
    """Tra ve so luong hoc sinh moi lop tu database noi bo"""
    conn = get_db()
    rows = conn.execute(
        "SELECT lop, COUNT(*) as total FROM students GROUP BY lop ORDER BY lop"
    ).fetchall()
    conn.close()
    result = {r['lop']: r['total'] for r in rows}
